            ...
            {isLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {Array.from({ length: 8 }).map((_, index) => (
                  <Card key={index}>
                    <Skeleton className="rounded-t-lg aspect-[1/1.25]" />
                ...
                {smartphones.map((phone) => {
                  ...
                  return (
                    <Card key={phone.id} className="flex flex-col h-full">
                      <img src={phone.image_url || 'https://placehold.co/400x500'} alt={phone.name} className="rounded-t-lg aspect-[1/1.25] object-contain bg-white dark:bg-muted p-4" />
