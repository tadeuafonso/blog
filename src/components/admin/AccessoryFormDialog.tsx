...
export const AccessoryFormDialog = ({ accessory, open, onOpenChange, onSave }) => {
  ...
  useEffect(() => {
    if (accessory) {
      setTitle(accessory.title || "");
      setRating(accessory.rating?.toString().replace('.', ',') || "");
      setStatus(accessory.status || "Rascunho");
      setImage(accessory.image || "");
      setTags(accessory.tags || []);
      setSummary(accessory.summary || "");
      setPros(accessory.pros?.join("\n") || "");
      setCons(accessory.cons?.join("\n") || "");
      setConclusion(accessory.conclusion || "");
      setAffiliateLinkAmazon(accessory.affiliate_link_amazon || "");
      setAffiliateLinkMl(accessory.affiliate_link_ml || "");
    } else {
      setTitle("");
      setRating("");
      setStatus("Rascunho");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{accessory ? "Editar Acessório" : "Adicionar Novo Acessório"}</DialogTitle>
        </DialogHeader>
